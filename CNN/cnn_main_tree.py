...
from Tree.Models.CNN_model import CNN
...
for k, v in sorted(trains.items(), reverse= True):
    ...
    if str(k[1]) == str(root_id) and str(k[2]) == str(root_id):
        for mod in ['cnn']: #['w_lstm', 'w_ch_lstm', 'transformer']:    
...
while todo != [] or len(children) != 0:
    ...
    if len(todo) != 0 and -1 in running.values(): # there at least one free GPU (for me)
            ...
            if pid == 0:
                ...
                execlp('python3.9', 'python3.9', '/home/tbidewell/home/POS_tagging/code/scripts/CNN/cnn_run_tree.py', destination, mod, parent_model, train, dev, test, gpu, number_of_repeats)
                exit()
